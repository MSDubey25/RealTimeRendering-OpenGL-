...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Direct3D11 Two 2D Shapes Color Animation"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
HRESULT initialize()
{
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Triangle, NULL, &gpID3D11Buffer_VertexBuffer_Triangle);
	if (FAILED(hr))
	{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResource_traingle;
	ZeroMemory(&mappedSubResource_traingle, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_traingle);
	memcpy(mappedSubResource_traingle.pData, vertices, sizeof(vertices));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Triangle, NULL);

	//for rectangle
	D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Rectangle;
	ZeroMemory(&bufferDesc_VertexBuffer_Rectangle, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer_Rectangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer_Rectangle.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesRectangle);
	bufferDesc_VertexBuffer_Rectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer_Rectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Rectangle, NULL, &gpID3D11Buffer_VertexBuffer_Rectangle);
	if (FAILED(hr))
	{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResource_rectangle;
	ZeroMemory(&mappedSubResource_rectangle, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_rectangle);
	memcpy(mappedSubResource_rectangle.pData, verticesRectangle, sizeof(verticesRectangle));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Rectangle, NULL);

	//for color
	//create Vertex buffer
	D3D11_BUFFER_DESC bufferDesc_ColorBuffer_Triangle;
	ZeroMemory(&bufferDesc_ColorBuffer_Triangle, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ColorBuffer_Triangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_ColorBuffer_Triangle.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesColor);
	bufferDesc_ColorBuffer_Triangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_ColorBuffer_Triangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_ColorBuffer_Triangle, NULL, &gpID3D11Buffer_ColorBuffer_Triangle);
	if (FAILED(hr))
	{

		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "ID3D11Device::CreateBufferColorTriangle() Failed...\n");
		fclose(msd_gpFile);
	}
	else
	{
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "ID3D11Device::CreateBufferColorTriangle() Succeded...\n");
		fclose(msd_gpFile);

	}
	D3D11_BUFFER_DESC bufferDesc_ColorBuffer_Rectangle;
	ZeroMemory(&bufferDesc_ColorBuffer_Rectangle, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ColorBuffer_Rectangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_ColorBuffer_Rectangle.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesRectangleColor);
	bufferDesc_ColorBuffer_Rectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_ColorBuffer_Rectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_ColorBuffer_Rectangle, NULL, &gpID3D11Buffer_ColorBuffer_Rectangle);
	if (FAILED(hr))
	{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResource_color_Rectangle;
	ZeroMemory(&mappedSubResource_color_Rectangle,sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Rectangle,0,
		D3D11_MAP_WRITE_DISCARD,0,&mappedSubResource_color_Rectangle);
	memcpy(mappedSubResource_color_Rectangle.pData,verticesRectangleColor,sizeof(verticesRectangleColor));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Rectangle,NULL);

	D3D11_MAPPED_SUBRESOURCE mappedSubResource_color_Triangle;
	ZeroMemory(&mappedSubResource_color_Triangle, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Triangle, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_color_Triangle);
	memcpy(mappedSubResource_color_Triangle.pData, verticesColor, sizeof(verticesColor));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Triangle, NULL);
...
void display()
{
	...
	UINT offset = 0;
	msd_gpID3D11DeviceContext->IASetVertexBuffers(0,1,&gpID3D11Buffer_VertexBuffer_Triangle,
		&stride,&offset);
	msd_gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_ColorBuffer_Triangle,
		&stride, &offset);
	//set topology
	...
	translationMatrix = XMMatrixTranslation(-2.0f, 0.0f, 5.0f);
	...
	//for REctangle
	msd_gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Rectangle,
		&stride, &offset);
	msd_gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_ColorBuffer_Rectangle,
		&stride, &offset);
	//set topology
	msd_gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
