...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Teapot With Texture With Light"),
...
int initialize()
{
	...
	else
	{
		...
		"		float tNorm_dot_Ld = (max(dot(normalizedlightDirection, normalizedtNorm), 0.0));"\
		"		vec3 reflectionVector = reflect(-normalizedlightDirection, normalizedtNorm);"\
		"		vec3 normalizedviewerVector = normalize(viewerVector);"\
		"		vec3 ambient = u_la;"\
		"		vec3 diffuse = u_ld * tNorm_dot_Ld;"\
		"		vec3 specular = vec3((max(dot(reflectionVector, normalizedviewerVector),0.0)));"\
		...
		"	}"\
		"fragColor = texture(u_sampler, outTexCoord) * vec4(phongADSLight, 1.0);"\
		"}";
		...
		glGenBuffers(1, &vbo_normal_teapot);
		glBindBuffer(GL_ARRAY_BUFFER, vbo_normal_teapot);
		glBufferData(GL_ARRAY_BUFFER, sizeof(float) * lengthOfArray, teapotNormals, GL_STATIC_DRAW);
...
void uninitialize()
{
	...
	if(vbo_normal_teapot)
	{
		glDeleteBuffers(1, &vbo_normal_teapot);
		vbo_normal_teapot = 0;
	}
