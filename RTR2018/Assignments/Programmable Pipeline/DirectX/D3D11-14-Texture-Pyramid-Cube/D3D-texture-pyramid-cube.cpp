...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Direct3D11 3D Texture Stone Kundali"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
HRESULT initialize()
{
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_texture, NULL, &gpID3D11Buffer_VertexBuffer_Texture_Pyramid);
	if (FAILED(hr))
	{
	...
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture_Pyramid, 0,
	...
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture_Pyramid, NULL);
	...
	//for texture
	//create Vertex buffer
	D3D11_BUFFER_DESC bufferDesc_cube_texture;
	ZeroMemory(&bufferDesc_cube_texture, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_cube_texture.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_cube_texture.ByteWidth = sizeof(float)*_ARRAYSIZE(cubeTexcoord);
	bufferDesc_cube_texture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_cube_texture.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_cube_texture, NULL, &gpID3D11Buffer_VertexBuffer_Texture_Cube);
	if (FAILED(hr))
	{
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubResource_cube_tex;
	ZeroMemory(&mappedSubResource_cube_tex, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture_Cube, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_cube_tex);
	memcpy(mappedSubResource_cube_tex.pData, cubeTexcoord, sizeof(cubeTexcoord));
	...
	if (FAILED(hr))
	{
		...
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "LoadD3DTexture() Cube Failed...\n");
		fclose(msd_gpFile);
	}
	else
	{
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "LoadD3DTexture() Cube Succeded...\n");
		fclose(msd_gpFile);
	...
	if (FAILED(hr))
	{
		...
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "LoadD3DTexture() Pyramid Failed...\n");
		fclose(msd_gpFile);
	}
	else
	{
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "LoadD3DTexture() Pyramid Succeded...\n");
		fclose(msd_gpFile);
...
void display()
{
	...
	///Pryamid
	stride = sizeof(float) * 3;//for x,y,z
	offset = 0;
	msd_gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Pyramid,
		&stride, &offset);
	stride = sizeof(float) * 2;//for u and v
	offset = 0;
	msd_gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Texture_Pyramid,
		&stride, &offset);

	//bind texture and sampler as a pixel shader resource
	msd_gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_Pyramid);
	msd_gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Texture_Pyramid);
