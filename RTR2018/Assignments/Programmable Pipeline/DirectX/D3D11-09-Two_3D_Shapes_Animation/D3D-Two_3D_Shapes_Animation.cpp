...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Direct3D11 Two 3D Shapes Animation"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
HRESULT initialize()
{
	...
	D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Pyramid;
	ZeroMemory(&bufferDesc_VertexBuffer_Pyramid, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer_Pyramid.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer_Pyramid.ByteWidth = sizeof(float)*_ARRAYSIZE(vertices);
	bufferDesc_VertexBuffer_Pyramid.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer_Pyramid.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Pyramid, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid);
	if (FAILED(hr))
	{
	...
	bufferDesc_VertexBuffer_Cube.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesCube);
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube, NULL, &gpID3D11Buffer_VertexBuffer_Rectangle);
	if (FAILED(hr))
	{
	...
	memcpy(mappedSubResource_cube.pData, verticesCube, sizeof(verticesCube));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Rectangle, NULL);

	//for color
	//create Vertex buffer
	D3D11_BUFFER_DESC bufferDesc_ColorBuffer_Pyramid;
	ZeroMemory(&bufferDesc_ColorBuffer_Pyramid, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ColorBuffer_Pyramid.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_ColorBuffer_Pyramid.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesColor);
	bufferDesc_ColorBuffer_Pyramid.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_ColorBuffer_Pyramid.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_ColorBuffer_Pyramid, NULL, &gpID3D11Buffer_ColorBuffer_Pyramid);
	if (FAILED(hr))
	{
	...
	D3D11_BUFFER_DESC bufferDesc_ColorBuffer_Cube;
	ZeroMemory(&bufferDesc_ColorBuffer_Cube, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ColorBuffer_Cube.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_ColorBuffer_Cube.ByteWidth = sizeof(float)*_ARRAYSIZE(verticesCubeColor);
	bufferDesc_ColorBuffer_Cube.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_ColorBuffer_Cube.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_ColorBuffer_Cube, NULL, &gpID3D11Buffer_ColorBuffer_Rectangle);
	if (FAILED(hr))
	{
	...
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Rectangle,0,
		D3D11_MAP_WRITE_DISCARD,0,&mappedSubResource_color_cube);
	memcpy(mappedSubResource_color_cube.pData,verticesCubeColor,sizeof(verticesCubeColor));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Rectangle,NULL);

	D3D11_MAPPED_SUBRESOURCE mappedSubResource_color_Pyramid;
	ZeroMemory(&mappedSubResource_color_Pyramid, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Pyramid, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_color_Pyramid);
	memcpy(mappedSubResource_color_Pyramid.pData, verticesColor, sizeof(verticesColor));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Pyramid, NULL);
...
void display()
{
	...
	msd_gpID3D11DeviceContext->IASetVertexBuffers(0,1,&gpID3D11Buffer_VertexBuffer_Pyramid,
		&stride,&offset);
	msd_gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_ColorBuffer_Pyramid,
		&stride, &offset);
	//set topology
	...
	rotationMatrix = XMMatrixRotationX(-msd_angle);
	rotationMatrix = rotationMatrix* XMMatrixRotationY(-msd_angle);
	rotationMatrix = rotationMatrix*XMMatrixRotationZ(-msd_angle);
	worldMatrix = rotationMatrix * translationMatrix * worldMatrix;
