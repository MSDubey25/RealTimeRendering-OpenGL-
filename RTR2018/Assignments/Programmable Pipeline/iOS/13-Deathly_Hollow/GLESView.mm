...
-(void)drawView:(id)sender
{
    ...
    static GLfloat msd_angle = 0.0f;
    static GLfloat msd_counter = 0.0f;
    bool msd_flag = false;
    GLfloat msd_ox, msd_oy, msd_radius, msd_perimeter, msd_a, msd_b, msd_c, msd_semi, msd_area;
    GLfloat msd_x = 0.5f, msd_y = 0.5f, msd_Sx = 1.0f, msd_Sy = 1.0f;
    GLfloat msd_linesCoordinates[1000];
    GLfloat msd_colorCoordinates[1000];
    ...
    //do necessary transformation
    msd_translationMatrix = vmath::translate(-msd_Sx + msd_counter, -msd_Sy + msd_counter, -3.0f);
    ...
    GLfloat msd_linesCoordinates1[1000];
    GLfloat msd_colorCoordinates1[1000];
    
    msd_linesCoordinates[0] = 0.0f;
    msd_linesCoordinates[1] = msd_y;
    msd_linesCoordinates[2] = 0.0f;
    msd_linesCoordinates[3] = -msd_x;
    msd_linesCoordinates[4] = -msd_y;
    msd_linesCoordinates[5] = 0.0f;
    msd_linesCoordinates[6] = msd_x;
    msd_linesCoordinates[7] = -msd_y;
    msd_linesCoordinates[8] = 0.0f;
    for (int i = 0; i < 9; i = i + 3)
    {
        msd_colorCoordinates[i + 0] = 1.0f;
        msd_colorCoordinates[i + 1] = 1.0f;
        msd_colorCoordinates[i + 2] = 1.0f;
    }
    ...
    msd_translationMatrix = vmath::translate(msd_Sx - msd_counter, -msd_Sy + msd_counter, -3.0f);
    msd_rotationMatrix = vmath::rotate( 0.0f, msd_angle, 0.0f);
    msd_modelViewMatrix = msd_modelViewMatrix*msd_translationMatrix;
    ...
    glUniformMatrix4fv(msd_mvpUniform, 1, GL_FALSE, msd_modelViewProjectionMatrix);
    //incircle
    msd_a = sqrtf((0.0f + msd_x)*(0.0f + msd_x) + (msd_y + msd_y)*(msd_y + msd_y));
    msd_b = sqrtf((msd_x + msd_x)*(msd_x + msd_x) + (-msd_y + msd_y)*(-msd_y + msd_y));
    msd_c = sqrtf((0.0f - msd_x)*(0.0f - msd_x) + (msd_y + msd_y)*(msd_y + msd_y));
    msd_perimeter = msd_a + msd_b + msd_c;
    msd_semi = msd_perimeter / 2;
    msd_ox = (msd_b*0.0f + msd_c * (-msd_x) + msd_a * msd_x) / msd_perimeter;
    msd_oy = (msd_b*msd_y + msd_c * (-msd_y) + msd_a * (-msd_y)) / msd_perimeter;
    
    msd_area = sqrtf(msd_semi*(msd_semi - msd_a)*(msd_semi - msd_b)*(msd_semi - msd_c));
    
    msd_radius = 2 * msd_area / msd_perimeter;
    for (int i = 0; i < 1000; i++)
    {
        
        GLfloat msd_angle = 2 * i* M_PI / num;
        if (i % 3 == 0)
        {
            msd_linesCoordinates1[i] = msd_radius * cos(msd_angle) + msd_ox;
            msd_colorCoordinates1[i] = 0.0f;
        }
        else if (i % 3 == 1)
        {
            msd_linesCoordinates1[i] = msd_radius * sin(msd_angle) + msd_oy;
            msd_colorCoordinates1[i] = 1.0f;
        }
        else
        {
            msd_linesCoordinates1[i] = 0.0f;
            msd_colorCoordinates1[i] = 1.0f;
        }
    }
    glBindBuffer(GL_ARRAY_BUFFER, msd_vbo_position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(msd_linesCoordinates1),
                 msd_linesCoordinates1, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindBuffer(GL_ARRAY_BUFFER, msd_vbo_color);
    glBufferData(GL_ARRAY_BUFFER, sizeof(msd_colorCoordinates1),
                 msd_colorCoordinates1, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindVertexArray(msd_vao);
    //glPointSize(2.0f);
    ...
    msd_translationMatrix = vmath::mat4::identity();
    //rotationMatrix = vmath::mat4::identity();
    msd_translationMatrix = vmath::translate(0.0f, msd_Sy - msd_counter, -3.0f);
    msd_modelViewMatrix = msd_modelViewMatrix * msd_translationMatrix;
    //do necessary matrix multiplication
    msd_modelViewProjectionMatrix = msd_perspectiveProjectionMatrix * msd_modelViewMatrix;
    ...
    msd_angle = msd_angle + 3.0f;
    if (msd_angle >= 360.0f)
        msd_angle = 0.0f;
    if (msd_counter <= 1.0f && msd_flag == false && msd_counter >= 0.0f)
        msd_counter = msd_counter + 0.015f;
    else
    {
        msd_flag = true;
    }
