    ...
    window=[[NSWindow alloc] initWithContentRect:win_rect
            ...
            NSWindowStyleMaskResizable
                                         backing:NSBackingStoreBuffered
                                           defer:NO];
    [window setTitle:@"macOS Dynamic India"];
    [window center];
...
-(void)drawView
{
    ...
    translationMatrix = vmath::translate(-2.0f+counter1,0.0f,-3.0f);
    ...
    translationMatrix = vmath::mat4::identity();
    modelViewMatrix = vmath::mat4::identity();
    translationMatrix = vmath::translate(0.2f, 6.5f-counter3, -3.0f);
    ...
    translationMatrix = vmath::mat4::identity();
    modelViewMatrix = vmath::mat4::identity();
    translationMatrix = vmath::translate(1.85f, -2.5f+counter4, -3.0f);
    modelViewMatrix = translationMatrix *scaleMatrix* modelViewMatrix;
    //do necessary matrix multiplication
    modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
    ...
    translationMatrix = vmath::mat4::identity();
    modelViewMatrix = vmath::mat4::identity();
    translationMatrix = vmath::translate(2.15f - counter2, 0.0f, -3.0f);
    modelViewMatrix = translationMatrix * scaleMatrix*modelViewMatrix;
    //do necessary matrix multiplication
    modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
    ...
    {
        modelViewMatrix = vmath::mat4::identity();
        translationMatrix = vmath::translate(1.13f, 0.0f, -3.0f);
        modelViewMatrix = translationMatrix * scaleMatrix*modelViewMatrix;
        //do necessary matrix multiplication
        modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
        ...
        modelViewMatrix = vmath::mat4::identity();
        translationMatrix = vmath::translate(1.235f, 0.0f, -3.0f);
 ...
 /*   translationMatrix=vmath::mat4::identity();
    ...
    if (xTranslate <=1.25f)
    {
        translationMatrix=vmath::translate(-4.5f + 2.4f*counterP1 + 1.0f, sin((counterP1*2.0f - 2.0f) / 2.0f) / 0.5f - 0.5f, -3.0f);
    }
    else
    {
        translationMatrix=vmath::translate(-4.5f + 2.4f*xTranslate + 1.0f, 0.0f, -3.0f);
    }
    if(xTranslate>2.09f)
    {
        translationMatrix=vmath::translate(-4.5f + 2.4f*counterP1 + 1.5f, sin((counterP1*2.0f - 2.0f) / 2.0f) / 0.5f - 0.5f, 0.0f);
    }
    modelViewMatrix = translationMatrix *modelViewMatrix;
    //do necessary matrix multiplication
    modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
...
-(void)DrawPlane
{
    ...
    const GLfloat AColor[] = {
        ...
        0.071f, 0.533f, 0.027f,//A
        0.071f, 0.533f, 0.027f,//A
        70.0f / 255.0f, 130.0f / 255.0f, 180.0f / 255.0f,    //A
   ...
   /* if (xTranslate <=1.25f)
    ...
    {
        translationMatrix=vmath::translate(-4.5f + 2.4f*xTranslate + 1.0f, 0.0f, 0.0f);
    }
    if(xTranslate>2.09f)
    {
        translationMatrix=vmath::translate(-4.5f + 2.4f*counterP1 + 1.5f, sin((counterP1*2.0f - 2.0f) / 2.0f) / 0.5f - 0.5f, 0.0f);
    ...
    */
    ...
    glUniformMatrix4fv(mvpUniform, 1, GL_FALSE, modelViewProjectionMatrix);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_position_letters);
    glBufferData(GL_ARRAY_BUFFER, sizeof(plane),
                 plane, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    //translationMatrix=vmath::mat4::identity();;
    scaleMatrix=vmath::mat4::identity();
    scaleMatrix=vmath::scale(0.1f, 0.05f, 0.0f);
    //translationMatrix=vmath::translate(1.0f,0.0f,-3.0f);
