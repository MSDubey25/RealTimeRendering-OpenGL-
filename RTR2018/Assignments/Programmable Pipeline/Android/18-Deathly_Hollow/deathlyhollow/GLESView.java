package com.astromedicomp.deathlyhollow;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener
{
	...
	//upto wglMakeCurrent is taken care by GLSurfaceView
		...
		//triangle vertices
		float traingleVertices[]=new float[2000];
		float traingleColor[]=new float[2000];
		//create vao triangle
	...
	{
		boolean flag = false;
		float ox, oy, radius, perimeter, a, b, c, semi, area;
		float x = 0.5f, y = 0.5f, Sx = 1.0f, Sy = 1.0f;
		float linesCoordinates[]=new float[2000];
		float colorCoordinates[]=new float[2000];
		...
		Matrix.translateM(translationMatrix,0,-Sx + counter, -Sy + counter, -3.0f);
		...
		//for incircle mathematically calculated
		//incircle
		a = (float)Math.sqrt((0.0f + x)*(0.0f + x) + (y + y)*(y + y));
		b = (float)Math.sqrt((x + x)*(x + x) + (-y + y)*(-y + y));
		c = (float)Math.sqrt((0.0f - x)*(0.0f - x) + (y + y)*(y + y));
		perimeter = a + b + c;
		semi = perimeter / 2;
		...
		for (int i = 0; i < 2000; i++)
		{
			...
			float angle = (float)(2 * i* Math.PI / num);
			if (i % 3 == 0)
			{
				linesCoordinates[i] = radius * (float)Math.cos(angle) + ox;
				colorCoordinates[i] = 0.0f;
			}
			else if (i % 3 == 1)
			{
				linesCoordinates[i] = radius * (float)Math.sin(angle) + oy;
				colorCoordinates[i] = 1.0f;
			}
			else
			{
				linesCoordinates[i] = 0.0f;
				colorCoordinates[i] = 1.0f;
			}
		}
		...
		for (int i = 0; i < 2000; i++)
		{
			...
			if (i % 3 == 0)
			{
				linesCoordinates[i] = (float)Math.cos(angle) / 1.15f;
				colorCoordinates[i] = 1.0f;
			}
			else if (i % 3 == 1)
			{
				linesCoordinates[i] = (float)Math.sin(angle) / 1.15f;
				colorCoordinates[i] = 1.0f;
			}
			else
			{
		...
		Matrix.translateM(translationMatrix,0,0,Sy - counter, -3.0f);
		//Matrix.rotateM(rotationMatrix,0,angle,0.0F,1.0F,0.0F);
