...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("D3D11 Diffuse Light on Sphere"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		"		float3 tnorm=(float3)normalize(mul(worldViewMatrix,normal));" \
	...
	const char *pixelShaderSourceCode =
		"float4 main(float4 pos:SV_POSITION, float4 diffuse_light:COLOR): SV_TARGET" \
		"{" \
		"float4 color=diffuse_light;" \
		"return color;" \
		"}";
	...
	fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
	fprintf_s(msd_gpFile, "no of vertices of sphere : %d...\n",gNumVertices);
	fprintf_s(msd_gpFile, "no of Elements of sphere : %d...\n", gNumElements);
	fclose(msd_gpFile);
	//for rectangle
	...
	bufferDesc_VertexBuffer_Cube.ByteWidth = sizeof(float)*_ARRAYSIZE(sphere_vertices);
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube, NULL, &gpID3D11Buffer_VertexBuffer_Sphere);
	if (FAILED(hr))
	{
	...
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Sphere, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_cube);
	memcpy(mappedSubResource_cube.pData, sphere_vertices, sizeof(sphere_vertices));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Sphere, NULL);
	...
	bufferDesc_NormalBuffer_Cube.ByteWidth = sizeof(float)*_ARRAYSIZE(sphere_normals);
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_NormalBuffer_Cube, NULL, &gpID3D11Buffer_NormalBuffer_Sphere);
	if (FAILED(hr))
	{
	...
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_NormalBuffer_Sphere,0,
		D3D11_MAP_WRITE_DISCARD,0,&mappedSubResource_normal_cube);
	memcpy(mappedSubResource_normal_cube.pData,sphere_normals, sizeof(sphere_normals));
	msd_gpID3D11DeviceContext->Unmap(gpID3D11Buffer_NormalBuffer_Sphere,NULL);
	...
	hr = msd_gpID3D11Device->CreateBuffer(&bufferDesc_IndexBuffer, NULL, &gpID3D11Buffer_IndexBuffer);
	if (FAILED(hr))
	{

		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "ID3D11Device::IndexBuffer() Failed...\n");
		fclose(msd_gpFile);
	}
	else
	{
		fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
		fprintf_s(msd_gpFile, "ID3D11Device::IndexBuffer() Succeded...\n");
		fclose(msd_gpFile);

	}
	fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
	fprintf_s(msd_gpFile, "After IndexBudffer..\n");
	fclose(msd_gpFile);

	D3D11_MAPPED_SUBRESOURCE mappedSubResource_index;
	ZeroMemory(&mappedSubResource_index, sizeof(D3D11_MAPPED_SUBRESOURCE));
	msd_gpID3D11DeviceContext->Map(gpID3D11Buffer_IndexBuffer, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource_index);
	memcpy(mappedSubResource_index.pData, sphere_elements, gNumElements*sizeof(short));
	...
	fopen_s(&msd_gpFile, msd_gszLogFileName, "a+");
	fprintf_s(msd_gpFile, "after index map..\n");
	fclose(msd_gpFile);
...
void display()
{
	...
	msd_gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Sphere,
		&stride, &offset);
	msd_gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_NormalBuffer_Sphere,
		&stride, &offset);
	msd_gpID3D11DeviceContext->IASetIndexBuffer(gpID3D11Buffer_IndexBuffer, DXGI_FORMAT_R16_UINT, 0);
