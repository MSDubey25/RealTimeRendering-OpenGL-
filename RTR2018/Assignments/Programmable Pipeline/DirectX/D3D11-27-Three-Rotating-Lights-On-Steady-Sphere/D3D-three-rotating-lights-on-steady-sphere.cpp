...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("D3D11 Three Rotating Lights On Steady Sphere"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode_perPixel =
		...
		"float3 lightDirection_red:NOMRAL2;"\
		"float3 lightDirection_blue:NOMRAL3;"\
		"float3 lightDirection_green:NOMRAL4;"\
		"};" \
		...
		"output.lightDirection_red=(float3)lightPosition_red-(float3)eyeCoordinate;" \
		"output.lightDirection_blue=(float3)lightPosition_blue-(float3)eyeCoordinate;" \
		"output.lightDirection_green=(float3)lightPosition_green-(float3)eyeCoordinate;" \
	...
	const char *pixelShaderSourceCode_perPixel =
		...
		"uint keypressed;" \
		"}" \
		"float4 main(float4 pos:SV_POSITION, float3 tnorm:NORMAL0, float3 viewVector:NORMAL1,float3 lightDirection_red:NORMAL2,float3 lightDirection_blue:NORMAL3, float3 lightDirection_green:NORMAL4): SV_TARGET" \
		"{" \
		"float4 color;" \
		"if(keypressed==1)" \
		"{" \
		"float3 tdnorm=normalize(tnorm);" \
		"float3 light_direction1=normalize(lightDirection_red);" \
		"float3 light_direction2=normalize(lightDirection_blue);" \
		"float3 light_direction3=normalize(lightDirection_green);" \
		"float tn_dot_ld_red=max(dot(tnorm,light_direction1),0.0);" \
		"float tn_dot_ld_blue=max(dot(tnorm,light_direction2),0.0);" \
		"float tn_dot_ld_green=max(dot(tnorm,light_direction3),0.0);" \
		"float3 reflectionVector_red=reflect(-light_direction1,tdnorm);" \
		"float3 reflectionVector_blue=reflect(-light_direction2,tdnorm);" \
		"float3 reflectionVector_green=reflect(-light_direction3,tdnorm);" \
		"float3 viewerVector=normalize(viewVector);" \
		"float4 ambient=la_red*ka+la_blue*ka+la_green*ka;"\
		"float4 diffuse=ld_red*kd*tn_dot_ld_red+ld_blue*kd*tn_dot_ld_blue+ld_green*kd*tn_dot_ld_green;" \
		"float4 specular=ls_red*ks*pow(max(dot(reflectionVector_red,viewerVector),0.0),material_shininess);" \
		"specular=specular+ls_blue*ks*pow(max(dot(reflectionVector_blue,viewerVector),0.0),material_shininess);" \
		"specular=specular+ls_green*ks*pow(max(dot(reflectionVector_green,viewerVector),0.0),material_shininess);" \
	...
	const char *vertexShaderSourceCode =
		...
		"float3 lightDirection_red=(float3)normalize(lightPosition_red-eyeCoordinate);" \
		"float3 lightDirection_blue=(float3)normalize(lightPosition_blue-eyeCoordinate);" \
		"float3 lightDirection_green=(float3)normalize(lightPosition_green-eyeCoordinate);" \
		"float tn_dot_ld_red=max(dot(tnorm,lightDirection_red),0.0);" \
		"float tn_dot_ld_blue=max(dot(tnorm,lightDirection_blue),0.0);" \
		"float tn_dot_ld_green=max(dot(tnorm,lightDirection_green),0.0);" \
		"float4 ambient=la_red*ka+la_blue*ka+la_green*ka;" \
		"float4 diffuse=ld_red*kd*tn_dot_ld_red+ld_blue*kd*tn_dot_ld_blue+ld_green*kd*tn_dot_ld_green;" \
		"float3 reflectionVector_red=reflect(-lightDirection_red,tnorm);" \
		"float3 reflectionVector_blue=reflect(-lightDirection_blue,tnorm);" \
		"float3 reflectionVector_green=reflect(-lightDirection_green,tnorm);" \
		"float3 viewVector=normalize(-eyeCoordinate.xyz);" \
		"float4 specular=ls_red*ks*pow(max(dot(reflectionVector_red,viewVector),0.0),material_shininess);" \
		"specular=specular+ls_blue*ks*pow(max(dot(reflectionVector_blue,viewVector),0.0),material_shininess);" \
		"specular=specular+ls_green*ks*pow(max(dot(reflectionVector_green,viewVector),0.0),material_shininess);" \
...
void display()
{
	...
	if (gbIsLight == true)
	{
		...
		constantBuffer.la_red = XMVectorSet(lights[0].LightAmbience[0], lights[0].LightAmbience[1], lights[0].LightAmbience[2], lights[0].LightAmbience[3]);
		...
		constantBuffer.la_blue = XMVectorSet(lights[1].LightAmbience[0], lights[1].LightAmbience[1], lights[1].LightAmbience[2], lights[1].LightAmbience[3]);
		constantBuffer.ld_blue = XMVectorSet(lights[1].LightDiffuse[0], lights[1].LightDiffuse[1], lights[1].LightDiffuse[2], lights[1].LightDiffuse[3]);
		constantBuffer.ls_blue = XMVectorSet(lights[1].LightSpecular[0], lights[1].LightSpecular[1], lights[1].LightSpecular[2], lights[1].LightSpecular[3]);
		constantBuffer.la_green = XMVectorSet(lights[2].LightAmbience[0], lights[2].LightAmbience[1], lights[2].LightAmbience[2], lights[2].LightAmbience[3]);
		constantBuffer.ld_green = XMVectorSet(lights[2].LightDiffuse[0], lights[2].LightDiffuse[1], lights[2].LightDiffuse[2], lights[2].LightDiffuse[3]);
		constantBuffer.ls_green = XMVectorSet(lights[2].LightSpecular[0], lights[2].LightSpecular[1], lights[2].LightSpecular[2], lights[2].LightSpecular[3]);
		...
		constantBuffer.lightPosition_blue = XMVectorSet(lights[1].LightPosition[0], lights[1].LightPosition[1], lights[1].LightPosition[2], lights[1].LightPosition[3]);
		constantBuffer.lightPosition_green = XMVectorSet(lights[2].LightPosition[0], lights[2].LightPosition[1], lights[2].LightPosition[2], lights[2].LightPosition[3]);
...
void Update()
{
	if (lights[0].angle >= 360.0f)
		...
		lights[0].angle += 0.002f;

	if (lights[1].angle >= 360.0f)
		lights[1].angle = 0.0f;
	else
		lights[1].angle += 0.002f;

	if (lights[2].angle >= 360.0f)
		lights[2].angle = 0.0f;
	else
		lights[2].angle += 0.002f;
}
