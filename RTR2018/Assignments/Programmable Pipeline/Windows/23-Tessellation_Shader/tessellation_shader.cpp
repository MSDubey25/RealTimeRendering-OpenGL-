...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Tessellation Shader"),
...
int initialize(void)
{
	...
	glGetShaderiv(gTessellationControlShaderObject,
		GL_COMPILE_STATUS,
		&iShaderCompileStatus);
	fwprintf(gpFile, TEXT("Value of igTessellationControlShaderObject shader compile Status is : %d\n"), iShaderCompileStatus);
	if (iShaderCompileStatus == GL_FALSE)
	{
		glGetShaderiv(gTessellationControlShaderObject,
			GL_INFO_LOG_LENGTH,
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				glGetShaderInfoLog(gTessellationControlShaderObject,
					...
					&writtened,
					szInfoLog);
				fwprintf(gpFile, TEXT("Error in compiling tessellation Control Shader : %hs\n"), szInfoLog);
				free(szInfoLog);
				uninitialize();
				//DestroyWindow(ghHwnd);
			}
	...
	glGetShaderiv(gTessellationEvaluationShaderObject,
		GL_COMPILE_STATUS,
		&iShaderCompileStatus);
	fwprintf(gpFile, TEXT("Value of igTessellationEvaluationShaderObject shader compile Status is : %d\n"), iShaderCompileStatus);
	if (iShaderCompileStatus == GL_FALSE)
	{
		glGetShaderiv(gTessellationEvaluationShaderObject,
			GL_INFO_LOG_LENGTH,
