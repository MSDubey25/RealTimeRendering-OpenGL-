...
void CreateWindow(void)
{
       ...
       XStoreName(gpDisplay,gWindow,"Geometry Shader Color");
...
void initialize(void)
{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				glGetShaderInfoLog(gVertexShaderObject,
					...
					&writtened,
					szInfoLog);
				printf("Error in compiling vertex Shader : %hs\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
	...
	glGetShaderiv(gGeometryShaderObject,
		GL_COMPILE_STATUS,
		&iShaderCompileStatus);
	printf("Value of iggeometryShaderObject shader compile Status is : %d\n", iShaderCompileStatus);
	if (iShaderCompileStatus == GL_FALSE)
	{
		glGetShaderiv(gGeometryShaderObject,
			GL_INFO_LOG_LENGTH,
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				glGetShaderInfoLog(gGeometryShaderObject,
					...
					&writtened,
					szInfoLog);
				printf("Error in compiling geometry Shader : %hs\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				glGetShaderInfoLog(gFragmentShaderObject,
					...
					&writtened,
					szInfoLog);
				printf("Error in compiling Fragment Shader : %hs\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				//DestroyWindow(ghHwnd);
			}
