...
@private
    ...
    GLuint msd_vao_triangle;
    GLuint msd_vao_rectangle;
    GLuint msd_vbo_position_triangle;
    GLuint msd_vbo_position_rectangle;
    GLuint msd_vbo_color_triangle;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        //create vao
        glGenVertexArrays(1, &msd_vao_triangle);
        glBindVertexArray(msd_vao_triangle);
        glGenBuffers(1, &msd_vbo_position_triangle);
        glBindBuffer(GL_ARRAY_BUFFER, msd_vbo_position_triangle);
        ...
        //for color
        glGenBuffers(1, &msd_vbo_color_triangle);
        glBindBuffer(GL_ARRAY_BUFFER, msd_vbo_color_triangle);
...
-(void)drawView:(id)sender
{
    ...
    msd_rotationMatrix = vmath::rotate(msd_angle_tri,0.0f,1.0f,0.0f);
...
-(void)dealloc
{
    if (msd_vbo_position_triangle)
    {
        glDeleteBuffers(1, &msd_vbo_position_triangle);
        msd_vbo_position_triangle = 0;
    }
    if (msd_vao_triangle)
    {
        glDeleteBuffers(1, &msd_vao_triangle);
        msd_vao_triangle = 0;
    }
    if (msd_vbo_color_triangle)
    {
        glDeleteBuffers(1, &msd_vbo_color_triangle);
        msd_vbo_color_triangle = 0;
    }
