...
function draw()
{
	...
	var flag = false;
	var ox, oy, radius, perimeter, a, b, c, semi, area;
	var x = 0.5, y = 0.5, Sx = 1.0, Sy = 1.0;
	
	modelViewMatrix=mat4.create();
	modelViewProjectionMatrix=mat4.create();
	
	mat4.translate(modelViewMatrix,modelViewMatrix,[-Sx + counter, -Sy + counter,-4.0]);
	//mat4.multiply(modelViewMatrix,modelViewMatrix,translationMatrix);
	mat4.multiply(modelViewProjectionMatrix,perspectiveProjectionMatrix,modelViewMatrix);
	gl.uniformMatrix4fv(mvpUniform,false,modelViewProjectionMatrix);
	
	var triangleVertices=new Float32Array([0.0,y,0.0,-x,-y,0.0,x,-y,0.0]);
	var linevertices=new Float32Array([0.0,y,0.0,0.0,-y,0.0]);
	...
	mat4.translate(modelViewMatrix,modelViewMatrix,[0,Sy - counter,0.0]);
	//mat4.multiply(modelViewMatrix,modelViewMatrix,translationMatrix);
	mat4.multiply(modelViewProjectionMatrix,perspectiveProjectionMatrix,modelViewMatrix);
	gl.uniformMatrix4fv(mvpUniform,false,modelViewProjectionMatrix);
	gl.bindBuffer(gl.ARRAY_BUFFER,vbo);
	...
	for(var i = 0.0, index = 0  ; i < 360.0 ; i = i + 1.0, index = index + 3)
	{
		...
		vertices[index] = 0.3 * Math.cos(degToRad);
		vertices[index + 1] = 0.3 * Math.sin(degToRad);
		vertices[index + 2] = 0.0;
	}
	
	mat4.translate(modelViewMatrix,modelViewMatrix,[Sx - counter, -Sy + counter-0.2,0.0]);
	mat4.rotateY(modelViewMatrix,modelViewMatrix,angle*Math.PI/180.0);
	//mat4.multiply(modelViewMatrix,modelViewMatrix,translationMatrix);
	mat4.multiply(modelViewProjectionMatrix,perspectiveProjectionMatrix,modelViewMatrix);
	gl.uniformMatrix4fv(mvpUniform,false,modelViewProjectionMatrix);
	gl.bindVertexArray(vao);
	...
	angle = angle + 3.0;
		...
		if (counter <= 1.0 && flag == false && counter >= 0.0)
			counter = counter + 0.011;
		else
		{
			flag = true;
		}
